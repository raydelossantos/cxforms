...
class AuthenticationController {
     ...
     */
    public function ldap_login(Request $request, Response $response, $args)
    {
        $result             = [];
        ...
        /**
         * Verify if user exists in users before authenticating via LDAP
         */
        ...
        if ($_user_check) {
             ...
             * Connect to LDAP
             */
            // $ldap_con =  ldap_connect($this->ldap_server);

            // if(!$ldap_con){
            //     $result['status'] = 'LDAP Error';
            //     $result['message'] = 'Cannot connect to Password Server. Please try again.';
             ...
             */
            // $ldap_binddn = "uid=$username,ou=users,dc=cloudstaff,dc=com";

            // $ldap_bind = @ldap_bind($ldap_con, $ldap_binddn, $password);
