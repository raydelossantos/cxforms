...
class AdminPrivilegeController {
    ...
    const SUCCESS = true;
    const ERROR = false;
    const SUCCESS_CREATE = 'Success. AdminPrivilege has been created.';
    const SUCCESS_UPDATE = 'Success. AdminPrivilege has been updated.';
    const SUCCESS_DELETE = 'Success, AdminPrivilege has been deleted.';
    const ERROR_SAVE = 'Error. AdminPrivilege was not saved.';
    const ERROR_UPDATE = 'Error. AdminPrivilege was not updated.';
    const ERROR_EXIST = 'Forbidden. AdminPrivilege already exists: [%s]';
    const ERROR_REQUIRED_FIELDS = 'Forbidden. Missing required parameters.';
    const ERROR_NOT_EXIST = 'Forbidden. You are updating AdminPrivilege that does not exists.';
    const ERROR_SAVING = 'Error. Something went wrong while saving AdminPrivilege.';
    const ERROR_BAD_REQUEST = 'Bad request.';
    const ERROR_DELETE = 'Error. Something went wrong while deleting AdminPrivilege.';
    ...
    public function get_all(Request $request, Response $response, $params)
    {
        ...
        if ( !empty($query) ) {
            $search = AdminPrivilege::where($query)->get();
        } else {
            $search = AdminPrivilege::all();
        }
    ...
    public function get(Request $request, Response $response, $params)
    {
        ...
        $search = AdminPrivilege::where('id', $params['id'])->first();
    ...
    public function create(Request $request, Response $response, $params) {
        ...
        if (!CommonController::validate(null, $request_data, ['display_name', 'user_id', 'username'])) {
            $result['message'] = self::ERROR_REQUIRED_FIELDS;
            return $response->withStatus(403)->withJson($result); 
        }

        $data = AdminPrivilege::create(array_map('trim', $request_data));
        if ($data) {
            $result['success'] = self::SUCCESS;
    ...
    public function update(Request $request, Response $response, $params) {
        ...
        // Update record
        $search = AdminPrivilege::find($params['id']);
        if ($search) {
            $data = $search->update(array_map('trim', $request_data));
    ...
    public function delete(Request $request, Response $response, $params) {
        ...
        $bot = AdminPrivilege::find($params['id']);
