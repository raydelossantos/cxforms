...
export class PublicFormComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  ngOnInit() {
    ...
    this.authService.getAuthCookiePublic();

    if (this.authService.authPublic === null) {
      this.router.navigate(['/public/error']);
      
      swal({
        position: 'top-end',
        type: 'error',
        title: 'An authorization error occured. Kindly close this window and try again.',
        focusConfirm: true,
        showCloseButton: false,
        confirmButtonText:
          '<i class="fa fa-thumbs-up"></i> Alright, let\'s go!'
      });
    ...
    this.user = {
      username:   this.authService.authPublic.user.user.username,
      user_id:    this.authService.authPublic.user.user.id,
      full_name:  this.authService.authPublic.user.user_info.first_name + ' ' + this.authService.authPublic.user.user_info.last_name,
      email:      this.authService.authPublic.user.user_info.email
    };
    ...
    this.route.params.subscribe(
      (params: Params) => {
        ...
        this.form_id = params['hash'];
        if (this.form_id !== null) {
          this.loading = true;
          this.formService.httpGetFormByHash(this.form_id);
        } else {
          this.router.navigate(['/public/error']);
        }
      }
  ...
  onSaveRecord() {
    ...
    if (valid.success) {
      ...
      this.defaultTableService.httpPostRecordPublic(this.form.id, postValues);
  ...
  onLoadEmployeeLookup(team_id: any, form_field_name: any) {
    ...
    this.memberService.httpGetAllMembersPublic({team_id: team_id});
